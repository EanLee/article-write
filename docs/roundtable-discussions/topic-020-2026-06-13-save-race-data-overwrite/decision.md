...
|---|---------|-------|-------|---------|------|
| 1 | 過渡防護：凍結自動儲存計時器＋寫入前比對 | 技術團隊 | P0 | 防護上線，舊快照無法覆寫磁碟 | ⏳ 待開始 |
| 2 | 儲存來源單一化實作（A 方案，含切換時儲存路徑） | 技術團隊 | P0 | 三路徑統一取編輯器即時內容，unit + E2E 通過 | ⏳ 待開始 |
| 3 | frontmatter 序列化欄位保留修復 | 技術團隊 | P0 | date/draft 等欄位儲存後不遺失，同分支 | ⏳ 待開始 |
| 4 | hash 比對＋寫入後驗證＋Sentry 事件 | Sam | P1 | 不一致時可在 Sentry 看到事件 | ⏳ 待開始 |
| 5 | 解除 test.fixme 並補三路徑 E2E | 技術團隊 | P1 | 驗收標準四項全過 | ⏳ 待開始 |
| 6 | 已知問題清單揭露文案 | Lisa | P2 | 措辭定稿並發布 | ⏳ 待開始 |
