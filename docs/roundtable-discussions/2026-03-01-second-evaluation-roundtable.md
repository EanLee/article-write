...
**效能工程師**：我想從效能角度補充 CRIT-01 的嚴重性。`FileScannerService` 是循序 I/O（P0-B），1000 個檔案要 500ms-2 秒——假設攻擊者或使用者誤觸發了掃描一個巨大目錄（比如 `/`），這不只是安全問題，還會完全凍結 Main Process，因為架構師說 `ConfigService` 用的是同步 I/O（W-04）。
...
**AI Token 工程師**：我想提一個大家報告裡都沒碰到的風險面——AI 模組。三個 Provider 的 Prompt 完全重複（DRY 嚴重違反）不是美學問題，是維護地雷：有人改了 Gemini 的 Prompt，忘記同步 Claude 和 OpenAI，使用者就會得到不一致的輸出。更嚴重的是 Gemini 沒有設 `maxOutputTokens`，如果使用者輸入很長的文章，Gemini 可能把整篇文章連同評論一起輸出，token 費用失控。

**資安工程師**：你提到的 Prompt Injection 風險我想接一下——使用者輸入直接進 Prompt，等於任何使用者都可以在文章裡寫 `Ignore all previous instructions and...` 來操控 AI 行為。在 Electron 應用裡這雖然主要是個人裝置，但如果哪天加了多人協作或雲端同步，這就是一個 B2 級漏洞。

**AI Token 工程師**：還有 `claude-haiku-4-5-20251001` 這個 model 名稱，我高度懷疑是不存在的，正確應該是 `claude-3-5-haiku-20241022`。如果 API key 是有效的但 model 名稱錯誤，Claude 的 SDK 會在執行時拋出 404，然後因為沒有 retry 邏輯，功能直接失效。無 Streaming 的問題讓這個現象更明顯——使用者等了 3-5 秒，看到的是一個 spinner 突然消失然後出現一個 API error。
...
**資安工程師** → **在 FileService 加入路徑白名單驗證**：一行防禦擊破 CRIT-01，所有後續 IPC 呼叫都繼承這個保護。

**效能工程師** → **讓 generateId() 回傳穩定的 UUID**：最低成本、最高 ROI，立即消除全量 DOM 重建。

**SOLID 工程師** → **修正 computed allTags 賦值 + updateArticle 方法名稱**：兩行程式碼，消除兩個 runtime crash。

**架構師** → **將 ipc-channels.ts 常數引入 main.ts 和 preload.ts**：強制型別安全覆蓋所有 IPC channel，防範未來的 typo 和 channel 偽造。

**AI Token 工程師** → **修正 Claude model 名稱為 claude-3-5-haiku-20241022**：一個字串修正，讓 AI 功能從「必定失敗」回到「可以測試」。

**程式品質工程師** → **為 FileService catch 保留原始 Error 物件**：讓所有 `ENOENT`/`EACCES` 可見，同時為資安偵測和除錯提供關鍵資訊。
...
> **共識行動項（依緊急度排序）**
> 1. 修正 `computed allTags` 賦值 + `updateArticle` 方法名稱（SOLID，今日）
> 2. 修正 Claude model 名稱（AI Token，今日）
> 3. FileService 路徑白名單 + 保留原始 Error（資安 + 品質，本週）
> 4. `generateId()` 穩定化（效能，本週）
> 5. `ipc-channels.ts` 常數統一引用（架構，本週）
> 6. 補 `MetadataCacheService`/`FileScannerService` 測試（品質，次週）
> 7. article.ts 拆解設計（架構 + SOLID，下個 Sprint）
> 8. markdown-it sanitize + sandbox 啟用（資安，下個 Sprint）
