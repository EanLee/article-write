...
|--------|------|------|
| M-01 | 中英文注釋統一 | 可維護 |
| M-07 | store 過度耦合 | SOLID、架構 |
| TS-ZERO | TypeScript 零錯誤清零目標（全 12 個）| 品質 |
...
| 中等問題數 | 15 個 | 7 個 | ⬆️ 改善 |
| TypeScript 錯誤數 | ~10（推估）| 12（精確統計）| ⬇️ 注意 |
| 測試通過數 | 373 | 373 | ➡️ |
| ESLint 錯誤 | 0 | 0 | ➡️ |
...
**最優先行動**:
1. 修正 S4-01（SearchPanel XSS 邊界）與 S4-02（exists/checkWritable 路徑驗證）
2. 設立 TypeScript 零錯誤 Sprint，清除 12 個已知錯誤
