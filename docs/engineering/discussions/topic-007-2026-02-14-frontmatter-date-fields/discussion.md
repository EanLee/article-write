...
> **發起日期**: 2026-02-14
> **狀態**: 🔄 討論中
> **相關話題**: [#006 發布機制落差確認](../../../roundtable-discussions/topic-007-2026-02-14-frontmatter-date-fields/2026-02-14-publish-mechanism-clarification.md)
...
**理由**:
1. 使用者在設定 frontmatter 時若看到 `date`，會直覺填入「今天」，結果可能不是他們想要的
2. 清楚的欄位命名降低使用者學習成本，有助推廣時的說明文件撰寫
3. 趁早期使用者少時改，比累積大量舊資料後再改，品牌影響小得多

**具體建議**:
- 在使用者說明文件中明確解釋三個欄位的語意差異
- 改動時配上自動移轉機制，讓使用者「無感升級」

**潛在風險/機會**:
- 風險：部落格端需要對應調整 schema，需要提供清楚的說明
- 機會：欄位語意清楚，日後的使用教學和推廣內容更容易撰寫
...
| Marketing (Lisa) | ✅ 支持 | 清楚命名降低學習成本，推廣文件更易撰寫 |
...
- [ ] Sam：更新 `docs/reference/dev-notes/GOTCHAS.md` 反映新欄位定義與移轉邏輯
- [ ] Sam：更新 `docs/engineering/E2E_PUBLISH_FLOW.md` 中的 frontmatter 說明

---

## 📌 需求追加（2026-02-14）

### Frontmatter 必要欄位自動初始化

**來源**：使用者需求追加（從時間欄位討論延伸）

**需求說明**：
系統開啟檔案時，若 frontmatter 中缺少必要欄位，應自動補上空值。目的是讓使用者在 WriteFlow 或 Obsidian 中都能看到完整的欄位結構，知道有哪些欄位可以填寫。
...
- 已有值 → 不覆蓋
- 缺少欄位 → 補上空值並寫回檔案
- 與時間欄位移轉在同一個 `migrateArticleFrontmatter()` 流程中執行

**行動項目**：
- [ ] Lin：在 `migrateArticleFrontmatter()` 中加入必要欄位初始化邏輯
