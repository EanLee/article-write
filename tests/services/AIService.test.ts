...
describe("AIService", () => {
  ...
  it("API Key 未設定時拋出 AIErrorCode.KeyMissing", async () => {
    ...
    await expect(aiService.generateSEO({ title: "測試", contentPreview: "內容" })).rejects.toMatchObject({ code: AIErrorCode.KeyMissing });
  });
  ...
  it("API 一般錯誤時拋出 AIErrorCode.ApiError", async () => {
    ...
    await expect(aiService.generateSEO({ title: "測試", contentPreview: "內容" }, "claude")).rejects.toMatchObject({ code: AIErrorCode.ApiError });
  });
  ...
  it("AIError 直接向上傳遞", async () => {
    ...
    await expect(aiService.generateSEO({ title: "測試", contentPreview: "內容" }, "claude")).rejects.toMatchObject({ code: AIErrorCode.Timeout });
  });
