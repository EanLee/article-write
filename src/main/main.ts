...
app.whenReady().then(() => {
  ...
  ipcMain.handle("set-config", (_, config: AppConfig) => configService.setConfig(config));
  ...
  // Publish Service
  ipcMain.handle("publish-article", async (event, article: Article, config: PublishConfig) => {
    // 注意：IPC 無法傳遞函式，不能利用 onProgress 參數
    // 轉掴為 event.sender.send 事件推播模式，讓 Renderer 端監聴 'publish-progress' 事件
    return await publishService.publishArticle(article, config, (step, progress) => {
      event.sender.send("publish-progress", { step, progress });
    });
  });

  ipcMain.handle("sync-all-published", async (event, config: PublishConfig) => {
