...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  // Publish operations
  publishArticle: (article: Record<string, unknown>, config: PublishConfig) => ipcRenderer.invoke("publish-article", article, config),
  ...
  syncAllPublished: (config: PublishConfig) => ipcRenderer.invoke("sync-all-published", config),
  ...
  gitCommit: (repoPath: string, options: { message: string; addAll?: boolean }) => ipcRenderer.invoke("git-commit", repoPath, options),
  gitPush: (repoPath: string, options?: { remote?: string; branch?: string }) => ipcRenderer.invoke("git-push", repoPath, options),
  gitAddCommitPush: (repoPath: string, commitMessage: string) => ipcRenderer.invoke("git-add-commit-push", repoPath, commitMessage),
  gitLog: (repoPath: string, count?: number) => ipcRenderer.invoke("git-log", repoPath, count),

  // Directory selection
  selectDirectory: (options?: { title?: string; defaultPath?: string }) => ipcRenderer.invoke("select-directory", options),
});
