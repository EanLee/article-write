...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  // Publish operations
  publishArticle: (article: Record<string, unknown>, config: PublishConfig) =>
  ...
  onPublishProgress: (callback: (data: { step: string; progress: number }) => void) => {
    ...
    return () => { ipcRenderer.removeListener(IPC.EVENT_PUBLISH_PROGRESS, listener); };
  },
  ...
  onSyncProgress: (callback: (data: { current: number; total: number; title: string }) => void) => {
    ...
    return () => { ipcRenderer.removeListener(IPC.EVENT_SYNC_PROGRESS, listener); };
  },
  ...
  onServerLog: (callback: (data: { log: string; type: "stdout" | "stderr"; timestamp: string }) => void) => {
    ...
    return () => { ipcRenderer.removeListener(IPC.EVENT_SERVER_LOG, listener); };
  },
  ...
  onFileChange: (callback: (data: { event: string; path: string }) => void) => {
    ...
    return () => { ipcRenderer.removeListener(IPC.EVENT_FILE_CHANGE, listener); };
  },
