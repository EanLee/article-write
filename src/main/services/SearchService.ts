...
export class SearchService {
  ...
  search(query: SearchQuery): SearchResult[] {
    ...
    const candidateIds = keyword.length < 3 ? new Set(this.index.keys()) : this.getCandidatesByTrigram(keyword);
    ...
    for (const id of candidateIds) {
      ...
      if (query.filters?.status && entry.status !== query.filters.status) {
        continue;
      }
      if (query.filters?.category && entry.category !== query.filters.category) {
        continue;
      }
      if (
        query.filters?.tags &&
        query.filters.tags.length > 0 &&
        !query.filters.tags.every((t) => entry.tags.some((tag) => tag.toLowerCase() === t.toLowerCase()))
      ) {
        continue;
      }
