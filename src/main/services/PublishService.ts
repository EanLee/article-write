...
export class PublishService {
   ...
   */
  async publishArticle(article: Article, config: PublishConfig, onProgress?: PublishProgressCallback): Promise<PublishResult> {
    const errors: string[] = [];
    ...
    try {
      ...
      // 處理圖片
      onProgress?.("處理圖片", 70);
      const { content: finalContent, imageWarnings } = await this.processImages(convertedContent, article, config);
   ...
   */
  async syncAllPublished(config: PublishConfig, onProgress?: (current: number, total: number, title: string) => void): Promise<SyncResult> {
    ...
    for (let i = 0; i < publishedArticles.length; i++) {
      ...
      if (publishResult.success) {
        ...
        if (publishResult.warnings) {
          result.warnings.push(...publishResult.warnings);
        }
      } else {
        result.failed++;
   ...
   * 處理圖片 - 複製圖片並更新路徑
   */
  private async processImages(content: string, article: Article, config: PublishConfig): Promise<{ content: string; imageWarnings: string[] }> {
    const warnings: string[] = [];
   ...
   * 寫入到 Astro 目錄
   */
  private async writeToAstro(article: Article, content: string, config: PublishConfig): Promise<string> {
