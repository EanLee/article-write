...
export class ProcessService {
  ...
  async startDevServer(projectPath: string): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      // QUAL6-07: 改用 pnpm（此專案使用 pnpm，不得用 npm）
      // S6-01: 移除 shell: true，改用平台明確二進位，防止 shell metacharacter 注入
      // Windows 上 pnpm 實際為 pnpm.cmd，直接指定以避免 shell 代理
      const pnpmBin = process.platform === "win32" ? "pnpm.cmd" : "pnpm";
      this.devServerProcess = spawn(pnpmBin, ["run", "dev"], {
        cwd: projectPath,
        stdio: "pipe",
      });
