...
export class ArticleService {
  ...
  async loadArticle(filePath: string, categoryFolder: string): Promise<Article> {
    ...
    // 決定文章分類：優先使用 frontmatter.category（使用者自訂），
    // 其次使用 frontmatter.categories[0]，最後使用資料夾名稱
    const articleCategory: string =
      frontmatter.category ||
      (frontmatter.categories && frontmatter.categories.length > 0 ? frontmatter.categories[0] : null) ||
      categoryFolder ||
      "uncategorized";
