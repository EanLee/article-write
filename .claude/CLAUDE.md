...
**合併前必須確認功能 end-to-end 可用，不只是 UI 有顯示。**
...
**同一需求內發現的問題**：
- 若實作過程發現需要修改其他層（型別、服務層等），應在**同一 branch 內**完成，不另開 branch
- 另開 branch 的情況：完全不相關的功能或重構
...
**❌ 驗證失敗**：
```bash
# ⚠️ 禁止另開新 branch！必須回到原 branch 繼續修復
git checkout fix/問題描述   # 或 feature/功能名稱
# ... 追加修復 ...
...
> **⚠️ RETRO-001 教訓**：使用者驗收回報問題後，不論問題大小（包含型別不相容、服務層缺漏等），都必須在**原 branch** 修復。另開新 branch 是錯誤的，會導致分支數量膨脹、歷史難以追蹤。
...
所有技術作業文件放在 `docs/tech-team/`，使用以下命名規則：
...
範例：
- `T-003-github-actions-cicd.md`
- `T-004-changelog-automation.md`
...
**⚠️ 重要：技術評估與方案討論必須在討論過程中同步建立文件，不得事後補寫**

#### 討論開始時

1. **立即建立 `T-XXX` 文件**，狀態設為 `📋 規劃中`
2. 將討論的問題背景寫入「任務背景」章節
3. 更新 `TEAM.md` 討論記錄索引

#### 討論進行中

- 將各成員的觀點與評估即時記錄至文件
- 方案比較表隨討論補充，不要等決策後才整理
