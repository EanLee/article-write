...
// 從 frontmatter 區塊抓 title／status（只需要純量字串欄位，不引入 YAML 套件）
...
function buildGroup(absDir, relDir, label) {
  ...
  for (const file of files) {
    ...
    if (fm.status === 'archived') archivedItems.push(item)
    else items.push(item)
  }

  // status: archived 的文件不離開原本 domain（domain＝主題歸屬，status＝生命週期，
  // 兩者是不同維度），而是收進該分類底下的「已封存文件」子分組
  if (archivedItems.length > 0) {
    items.push({ text: '已封存文件', items: archivedItems, collapsed: true })
  }
